...
CONTAINS
    ...
    FUNCTION func_range(t_min, t_max)
        IMPLICIT NONE
        REAL :: t_max, t_min, func_range

        func_range = 1.0/(t_max-t_min)
    ...
    FUNCTION func_norml()
        ...
        REAL :: t_max, t_min, func_range
        INTEGER :: i
        REAL, DIMENSION(1:2,1:10) :: p, pdash

            FOR i=1, n
                norm_const = (sum(pdash)-p5*(pdash(0)+pdash(n)))*dx
            END FOR
